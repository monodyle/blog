...
class FenceParser {
  ...
  metadata() {
    ...
    const parsingErrors = [];
    try {
      const pairs = this.input.filter(
        (token) => token.tokenType.name === "Pair"
      );
      ...
      const ranges = this.input.filter(
        (token) => token.tokenType.name === "Range"
      );
      if (ranges && ranges.length) {
        for (const range of ranges) {
          ...
          const values = range.image
            .substring(startIndex + 1, stopIndex)
            .split(",")
            .flatMap((token) => parseRange(token.trim()));
